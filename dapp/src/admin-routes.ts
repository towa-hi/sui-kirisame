...
export function adminRoutes(sui: SuiGrpcClient) {
  const routes = new Hono();
  routes.post('/:action', async c => {
    c.header('Cache-Control', 'no-store');
    const action = adminActions.find(item => item.id === c.req.param('action'));
    if (!action) return c.json({ error: 'Unknown admin function.' }, 404);
    let body;
    ...
    try {
      ...
      const args: TransactionArgument[] = [tx.object(objects[0].objectId)];
      for (const field of action.fields) {
      ...
      if (action.id === 'admin_settle_pending_payments') args.push(tx.object('0x6'));
