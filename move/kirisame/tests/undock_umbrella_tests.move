...
module kirisame::undock_umbrella_tests {
    ...
    fun run(state: u8, payment: u64, cycle: u64, wrong_station: bool, duplicate: bool) {
        ...
        if (state == 1) {
            umbrella::dock_umbrella(&admin, &cap, &station, &mut asset, 0, &clock, scenario.ctx());
        } else if (state > 1) {
            umbrella::prepare_return_for_testing(&mut asset, state, false);
        };
        umbrella::undock_umbrella(if (wrong_station) { &other } else { &station }, &mut asset,
        ...
        if (duplicate) {
            umbrella::undock_umbrella(&station, &mut asset, coin::mint_for_testing<SUI>(payment, scenario.ctx()), 1, &clock, scenario.ctx());
        };
        // Returning successfully verifies that purchase entered Held.
        clock.set_for_testing(deadline);
        umbrella::dock_umbrella(&admin, &cap, &station, &mut asset, 1, &clock, scenario.ctx());
        umbrella::undock_umbrella(&station, &mut asset, coin::mint_for_testing<SUI>(payment, scenario.ctx()), 1, &clock, scenario.ctx());
